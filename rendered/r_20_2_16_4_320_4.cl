__kernel void r_20_2_16_4_320_4(__global float* data0_2560, __global float* data1_2560, __global float* data2_1638400, __global float* data3_1280) {
  float acc0[4];
  ...
  *(acc0+2) = 0.0;
  *(acc0+3) = 0.0;
  for (int ridx1002 = 0; ridx1002 < 320; ridx1002++) {
    int alu5 = (ridx1002<<2);
  ...
  float alu13 = ((*(acc0+0))+val5.x);
  float alu14 = ((*(acc0+1))+val5.y);
  float alu15 = ((*(acc0+2))+val5.z);
  float alu16 = ((*(acc0+3))+val5.w);
  *((__global float4*)((data0_2560+(alu12+alu0)))) = (float4)((alu13*(1/(1.0+exp2((alu13*-1.4426950408889634))))),(alu14*(1/(1.0+exp2((alu14*-1.4426950408889634))))),(alu15*(1/(1.0+exp2((alu15*-1.4426950408889634))))),(alu16*(1/(1.0+exp2((alu16*-1.4426950408889634))))));
}