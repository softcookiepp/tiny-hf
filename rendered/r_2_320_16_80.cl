__kernel void r_2_320_16_80(__global float* data0_640, __global float* data1_2560, __global float* data2_409600, __global float* data3_320) {
  ...
  *(acc1+0) = 0.0;
  float val0 = (*(data3_320+gidx0));
  *(acc0+0) = 0.0;
  for (int ridx1003 = 0; ridx1003 < 80; ridx1003++) {
    int alu2 = ((lidx0*80)+ridx1003);
    float val1 = (*(data2_409600+(alu2+(gidx0*1280))));
  ...
  *(temp0+lidx0) = (*(acc0+0));
  ...
  if ((((bool)(lidx0))!=1)) {
    ...
    *(data0_640+(gidx0+(gidx1*320))) = ((*(acc1+0))+val0);
  }
}