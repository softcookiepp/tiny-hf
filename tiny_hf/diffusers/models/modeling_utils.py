...
def get_parameter_device(parameter: tga.nn.Module) -> str:#torch.device:
	...
	try:
		...
		def find_tensor_attributes(module: tga.nn.Module) -> List[Tuple[str, Tensor]]:
			tuples = [(k, v) for k, v in module.__dict__.items() if tga.is_tensor(v)]
			return tuples
...
def get_parameter_dtype(parameter: tga.nn.Module) -> tga.dtype:
	"""
...
def no_init_weights():
	...
	tga.nn.init function are all replaced with skip.
	"""
	...
	for name, init_func in TORCH_INIT_FUNCTIONS.items():
		setattr(tga.nn.init, name, _skip_init)
	try:
		yield
	finally:
		# Restore the original initialization functions
		for name, init_func in TORCH_INIT_FUNCTIONS.items():
			setattr(tga.nn.init, name, init_func)


class ModelMixin(tga.nn.Module, PushToHubMixin):
	r"""
	...
	def __getattr__(self, name: str) -> Any:
		...
		__getattr__ here in addition so that we don't trigger `tga.nn.Module`'s __getattr__':
		...
		*checkpoint activations* in other frameworks).
		...
		if gradient_checkpointing_func is None:
			...
			def _gradient_checkpointing_func(module, *args):
				...
				return tga.utils.checkpoint.checkpoint(
					module.__call__,
					*args,
					**ckpt_kwargs,
				)
	...
	def set_use_npu_flash_attention(self, valid: bool) -> None:
		...
		def fn_recursive_set_npu_flash_attention(module: tga.nn.Module):
		...
		for module in self.children():
			if isinstance(module, tga.nn.Module):
	...
	def set_use_xla_flash_attention(
		...
		# gets the message
		def fn_recursive_set_flash_attention(module: tga.nn.Module):
	...
	def set_use_memory_efficient_attention_xformers(
		...
		# gets the message
		def fn_recursive_set_mem_eff(module: tga.nn.Module):
	...
	def enable_layerwise_casting(
		self,
		storage_dtype: tga.dtype = None,#tga.float8_e4m3fn,
		compute_dtype: Optional[tga.dtype] = None,
		skip_modules_pattern: Optional[Tuple[str, ...]] = None,
		skip_modules_classes: Optional[Tuple[Type[tga.nn.Module], ...]] = None,
		...
		Example:
			...
			...     "THUDM/CogVideoX-5b", subfolder="transformer", torch_dtype=tga.bfloat16
			... )

			>>> # Enable layerwise casting via the model, which ignores certain modules by default
			>>> transformer.enable_layerwise_casting(storage_dtype=tga.float8_e4m3fn, compute_dtype=tga.bfloat16)
			```

		Args:
			storage_dtype (`tga.dtype`):
				The dtype to which the model should be cast for storage.
			compute_dtype (`tga.dtype`):
			...
			skip_modules_pattern (`Tuple[str, ...]`, *optional*):
				...
				layers.
			skip_modules_classes (`Tuple[Type[tga.nn.Module], ...]`, *optional*):
	...
	def enable_group_offload(
		self,
		onload_device: tga.device,
		offload_device: tga.device = tga.device("cpu"),
		...
		Example:
			...
			...     onload_device=tga.device("cuda"),
			...     offload_device=tga.device("cpu"),
		...
		**kwargs,
		...
		Arguments:
			...
			save_function (`Callable`):
				...
				replace `tga.save` with another method. Can be configured with the environment variable
				`DIFFUSERS_SAVE_MODE`.
		...
		for filename, tensors in state_dict_split.filename_to_tensors.items():
			...
			if safe_serialization:
				...
				tinygrad.nn.state.safe_save(shard, filepath, metadata={"format": "pt"})
			else:
	...
	def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs) -> Self:
		...
		Parameters:
			...
			cache_dir (`Union[str, os.PathLike]`, *optional*):
				...
				is not used.
			torch_dtype (`str` or `tga.dtype`, *optional*):
				Override the default `tga.dtype` and load the model with another dtype. If `"auto"` is passed, the
			...
			mirror (`str`, *optional*):
				...
				information.
			device_map (`str` or `Dict[str, Union[int, str, tga.device]]`, *optional*):
		...
		- conv_in.weight: found shape tga.Size([320, 4, 3, 3]) in the checkpoint and tga.Size([320, 9, 3, 3]) in the model instantiated
		...
		torch_dtype = kwargs.pop("torch_dtype", tga.float32)
		...
		if not isinstance(torch_dtype, tga.dtype):
			torch_dtype = tga.float32
			logger.warning(
				f"Passed `torch_dtype` {torch_dtype} is not a `tga.dtype`. Defaulting to `tga.float32`."
			)
		...
		# change device_map into a map if we passed an int, a str or a tga.device
		if isinstance(device_map, tga.device):
			device_map = {"": device_map}
		elif isinstance(device_map, str) and device_map not in ["auto", "balanced", "balanced_low_0", "sequential"]:
			try:
				device_map = {"": tga.device(device_map)}
			except RuntimeError:
				raise ValueError(
		...
		if low_cpu_mem_usage:
			if device_map is not None and not is_torch_version(">=", "1.10"):
				# The max memory utils require PyTorch >= 1.10 to have tga.cuda.mem_get_info.
		...
		if torch_dtype is not None and not torch_dtype == getattr(torch, "float8_e4m3fn", None):
			if not isinstance(torch_dtype, tga.dtype):
				raise ValueError(
					f"{torch_dtype} needs to be of type `tga.dtype`, e.g. `tga.float16`, but is {type(torch_dtype)}."
				)
		...
		if dtype_orig is not None:
			tga.set_default_dtype(dtype_orig)
		...
		if hf_quantizer is not None:
			...
			# the model config because `_pre_quantization_dtype` is `tga.dtype`, not JSON serializable.
	...
	@wraps(tga.nn.Module.cuda)
	def cuda(self, *args, **kwargs):
		# this is backend-api-specific code, not good...
		raise NotImplementedError
	...
	def to(self, *args, **kwargs):
		...
		# Try converting arguments to tga.device in case they are passed as strings
		for arg in args:
	...
	def _load_pretrained_model(
		...
		# disabling for now, as quantizer is not implemented
		hf_quantizer = None, #: Optional[DiffusersQuantizer] = None,
		low_cpu_mem_usage: bool = True,
		dtype: Optional[Union[str, tga.dtype]] = None,
		keep_in_fp32_modules: Optional[List[str]] = None,
		device_map: Dict[str, Union[int, str, tga.device]] = None,
	...
	@classmethod
	def _set_default_torch_dtype(cls, dtype: tga.dtype) -> tga.dtype:
		"""
		...
		Returns:
			`tga.dtype`: the original `dtype` that can be used to restore `tga.set_default_dtype(dtype)` if it was
		...
		dtype_orig = tga.get_default_dtype()
		tga.set_default_dtype(dtype)
		return dtype_orig

	@property
	def device(self) -> tga.device:
		"""
		`tga.device`: The device on which the module is (assuming that all the module parameters are on the same
		device).
		"""
		return get_parameter_device(self)

	@property
	def dtype(self) -> tga.dtype:
		"""
		`tga.dtype`: The dtype of the module (assuming that all the module parameters have the same dtype).
		"""
	...
	def get_memory_footprint(self, return_buffers=True):
		...
		PyTorch discussions: https://discuss.pytga.org/t/gpu-memory-that-model-uses/56822/2

		Arguments:
			return_buffers (`bool`, *optional*, defaults to `True`):
				Whether to return the size of the buffer tensors in the computation of the memory footprint. Buffers
				are tensors that do not require gradients and not registered as parameters. E.g. mean and std in batch
				norm layers. Please see: https://discuss.pytga.org/t/what-pytorch-means-by-buffers/120266/2
		"""
	...
	def _set_gradient_checkpointing(
		self, enable: bool = True, gradient_checkpointing_func = None#: Callable = tga.utils.checkpoint.checkpoint
	) -> None:
		raise NotImplementedError
