...
def _get_single_file_loadable_mapping_class(cls):
	diffusers_module = importlib.import_module(__name__.split(".")[0:2].join("."))
	input(diffusers_module)
...
class FromOriginalModelMixin:
	...
	def from_single_file(cls, pretrained_model_link_or_path_or_dict: Optional[str] = None, **kwargs) -> Self:
		...
		torch_dtype = kwargs.pop("torch_dtype", dtypes.float32)
		quantization_config = kwargs.pop("quantization_config", None)
		device = kwargs.pop("device", None)
		disable_mmap = kwargs.pop("disable_mmap", False)

		if not isinstance(torch_dtype, tinygrad.dtype.DType):
			torch_dtype = dtypes.float32
			logger.warning(
				f"Passed `torch_dtype` {torch_dtype} is not a `torch.dtype`. Defaulting to `dtypes.float32`."
			)
		...
		use_keep_in_fp32_modules = (cls._keep_in_fp32_modules is not None) and (
			(torch_dtype == dtypes.float16) or hasattr(hf_quantizer, "use_keep_in_fp32_modules")
		)
		...
		if is_accelerate_available():
			...
			param_device = device if (not device is None) else "CPU"
			...
			#print(type(diffusers_format_checkpoint[list(diffusers_format_checkpoint.keys())[0]]) )
			assert isinstance(model, tga.nn.Module)
			_, unexpected_keys = model.load_state_dict(diffusers_format_checkpoint, strict=False)
			#_, unexpected_keys = model.load_state_dict({10, "penis"}, strict=False)
		...
		if torch_dtype is not None and hf_quantizer is None:
			pass
			# this should be handled at an earlier stage, ideally
